...
export function NotificationsSection({ pendingProofs, pendingCash, recentSignedDevis }: NotificationsSectionProps) {
    ...
    return (
        <Card className="border-primary/15 bg-primary/5 shadow-card">
            <CardContent className="p-6">
                <h3 className="font-display font-semibold mb-4 flex items-center gap-2">
                    <Bell className="h-4 w-4 text-primary" /> Notifications & Actions requises
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {pendingProofs.map((proof: any) => (
                        <Link key={`proof-${proof.id}`} to={`/finance/factures/${proof.facture_id}`} className="block">
                            <div className="flex gap-3 rounded-[22px] border border-border/70 bg-card/95 p-3.5 shadow-sm transition-colors hover:border-info/40">
                                <div className="h-fit rounded-[14px] bg-info/10 p-2 text-info"><FileText className="w-4 h-4" /></div>
                                <div>
                    ...
                    {pendingCash.map((cash: any) => (
                        <Link key={`cash-${cash.id}`} to={`/finance/factures/${cash.facture_id}`} className="block">
                            <div className="flex gap-3 rounded-[22px] border border-border/70 bg-card/95 p-3.5 shadow-sm transition-colors hover:border-warning/40">
                                <div className="h-fit rounded-[14px] bg-warning/10 p-2 text-warning"><Banknote className="w-4 h-4" /></div>
                                <div>
                    ...
                    {recentSignedDevis.map((devis: any) => (
                        <Link key={`devis-${devis.id}`} to={`/finance/devis/${devis.id}`} className="block">
                            <div className="flex gap-3 rounded-[22px] border border-border/70 bg-card/95 p-3.5 shadow-sm transition-colors hover:border-success/40">
                                <div className="h-fit rounded-[14px] bg-success/10 p-2 text-success"><CheckCircle2 className="w-4 h-4" /></div>
                                <div>
